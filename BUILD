...
genrule(
    ...
    outs = ["psmisc_final_installed.tar"],
    cmd = COMMON_SCRIPT + ENTER_LFS_SCRIPT + """
        extract_dependency $(location image_initial_rootfs.tar)
        extract_dependency $(location gcc_final_installed.tar)
        extract_dependency $(location mpc_installed.tar)
        ...
        extract_source $(location @psmisc_src.tar//file)
...
genrule(
    ...
    outs = ["gettext_final_installed.tar"],
    cmd = COMMON_SCRIPT + ENTER_LFS_SCRIPT + """
        extract_dependency $(location image_initial_rootfs.tar)
        extract_dependency $(location gcc_final_installed.tar)
        extract_dependency $(location mpc_installed.tar)
...
genrule(
    ...
    cmd = COMMON_SCRIPT + ENTER_LFS_SCRIPT + """
        ...
        extract_source $(location @grep_src.tar//file)
        ...
        # Remove files other than the ones installed by grep
        cd $$START_DIR
        tar tf $(location grep_installed.tar) | grep -v '/$$' > files_to_keep.txt
        grep -v -f files_to_keep.txt extracted_files.txt > extracted_files.txt.tmp
        mv extracted_files.txt.tmp extracted_files.txt
...
genrule(
    ...
    cmd = COMMON_SCRIPT + ENTER_LFS_SCRIPT + """
        ...
        extract_source $(location @gdbm_src.tar//file)
...
genrule(
    ...
    cmd = COMMON_SCRIPT + ENTER_LFS_SCRIPT + """
        ...
        extract_source $(location @expat_src.tar//file)

        run_bash_script_in_lfs "
            cd /src
            ./configure --prefix=/usr --disable-static --docdir=/usr/share/doc/expat-{expat_version}
            make
            make check
            make install
...
genrule(
    ...
    outs = ["intltool_installed.tar"],
    cmd = COMMON_SCRIPT + ENTER_LFS_SCRIPT + """
        extract_dependency $(location image_initial_rootfs.tar)
        extract_dependency $(location perl_final_installed.tar)
        extract_dependency $(location xml_parser_installed.tar)
        extract_dependency $(location expat_installed.tar)
        extract_dependency $(location gcc_final_installed.tar)
        extract_dependency $(location mpc_installed.tar)
        extract_dependency $(location mpfr_installed.tar)
        ...
        extract_source $(location @intltool_src.tar//file)
        ...
        $$TAR -cf "$@" -C "$$LFS" .
    """,
    tags = ["ref=https://www.linuxfromscratch.org/lfs/view/12.2/chapter08/intltool.html"],
)
...
genrule(
    ...
    cmd = COMMON_SCRIPT + ENTER_LFS_SCRIPT + """
        ...
        $$TAR -cf "$@" -C "$$LFS" .
    """,
    tags = ["ref=https://www.linuxfromscratch.org/lfs/view/12.2/chapter08/libelf.html"],
)
